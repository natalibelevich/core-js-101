...
function getPolynom() {
  return (x) => {
    ...
    return Array.from(arguments).reverse().reduce((res, val, i) => res + val * Math.pow(x, i), 0);
  };
}
...
function logger(func, logFunc) {
  ...
  function parseArg(val) {
    ...
    if (val instanceof Array) {
      const rez = [];
      for (const i in val) rez.push(parseArg(val[i]));
      if (rez.length === 1) return rez[0].toString();
      return `[${rez.join(',')}]`;
    }
    return val.toString();
  }
